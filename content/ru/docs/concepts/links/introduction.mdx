---
title: "Введение в короткие ссылки"
...
- тысячи ссылок, программно и пачками, для вашей SMS-кампании — см. [массовые операции](/docs/concepts/links/bulk-operations)
...
| ----------- | :-------------------------------------------------------- | :------------------------------- |
| `id`        | Уникальный идентификатор ссылки (с префиксом `link_`)     | `link_1KJ1H3PZGE081A22TC6PTQYJP` |
| `url`       | Целевой URL, куда ведёт ссылка                            | `https://revroute.ru/`           |
| `shortLink` | Короткая версия ссылки, включая схему                     | `https://link.revroute.ru/claim`         |
| `domain`    | Домен короткой ссылки                                     | `link.revroute.ru`               |
| `key`       | Слаг короткой ссылки                                      | `claim`                          |

Полный список полей и правила их заполнения — в разделе [«API: введение»](/docs/api-reference/introduction).

Для отправки конверсионных событий по созданным ссылкам см. [«Как работает атрибуция»](/docs/concepts/attribution) и [клиентский скрипт RevRoute](/docs/sdks/client-side/introduction).
...
Параметры запроса и формат ответа описаны в разделе [«API: введение»](/docs/api-reference/introduction).
...
```bash cURL
curl --request PATCH \
  --url https://api.revroute.ru/links/link_1KJ1H3PZGE081A22TC6PTQYJP \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
...
Общие правила работы с API — аутентификация, коды ошибок, пагинация — описаны в разделе [«API: введение»](/docs/api-reference/introduction).
...
```bash cURL
curl --request POST \
  --url https://api.revroute.ru/links/upsert \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
  --data '{"url": "https://google.com"}'
```
