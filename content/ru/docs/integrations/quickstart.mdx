...
description: "Узнайте, как аутентифицировать пользователей с помощью OAuth 2.0 для вашей интеграции с RevRoute."
---
...
Уже готовые интеграции собраны в [каталоге интеграций](/integrations); эта страница — о том, как собрать свою.

Вы можете создавать собственные интеграции с RevRoute, используя наш [API](/docs/api-reference/introduction).
...
2. Разберитесь, как устроена [аутентификация](/docs/api-reference/authentication): секретный ключ, publishable key и токен OAuth.
...
<Steps>
  <Step title="Создайте OAuth2-приложение в RevRoute">
    - Перейдите на вкладку [OAuth Apps](https://app.revroute.ru/settings/oauth-apps) в вашем рабочем пространстве.
  ...
  <Step title="Перенаправьте пользователей на URL авторизации">
    ...
    | `scope` | Список областей доступа, разделённых пробелами, к которым вы хотите запросить доступ. Подробнее об областях доступа [здесь](#области-доступа-scopes). |
    ...
    ![Экран согласия OAuth](/images/oauth-consent.png)
  ...
  <Step title="Обменяйте код на токен доступа">
    ...
    <Tabs>
      <Tab title="С PKCE (рекомендуется)">
        ...
        Например, [расширение RevRoute для Raycast](https://github.com/raycast/extensions/tree/main/extensions/dub) использует PKCE для аутентификации пользователей.
      </Tab>
  ...
  <Step title="Выполните API-запрос с токеном доступа">
    ...
    Полученный токен передаётся в заголовке `Authorization` — так же, как обычный ключ API (см. [«API: аутентификация»](/docs/api-reference/authentication)).
...
Подробнее об [API-ключах](/docs/api-reference/authentication#секретные-apiключи-servertoserver).
