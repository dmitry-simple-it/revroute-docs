...
Then, the Revroute Analytics script detects the `dub_id` parameter and stores it as a **first-party cookie**.
...
the `dub_id` is lost on a hop, [Tracking issues](/docs/troubleshooting/tracking-issues#cors-errors)
