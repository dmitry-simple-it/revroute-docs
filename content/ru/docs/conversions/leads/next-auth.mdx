---
title: "NextAuth.js"
description: "Узнайте, как отслеживать события конверсий лидов с помощью NextAuth.js и Dub"
---
...
Под капотом Dub записывает пользователя как клиента и связывает его с событием клика, откуда он пришёл. Уникальный ID пользователя теперь является источником истины для всех будущих событий — поэтому cookie `dub_id` больше не нужен.
