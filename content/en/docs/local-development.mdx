...
description: "A guide on how to run Revroute's codebase locally."
---
...
Revroute's codebase is set up in a monorepo (via [Turborepo](https://turbo.build/repo)) and is fully [open-source on GitHub](https://github.com/dubinc/dub).
...
- `web`: The entirety of Revroute's application ([app.revroute.ru](https://app.revroute.ru)) + our link redirect infrastructure.

The `packages` directory contains the code for:

- `cli`: A CLI for easily shortening URLs with the Revroute API.
- `email`: Revroute's email application with function to send emails and templates.
- `embeds`: A package used embed Revroute's referral dashboard.
- `prisma`: Prisma Configuration for Revroute's web-app.
- `stripe-app`: The Stripe app for dub conversions.
- `tailwind-config`: The Tailwind CSS configuration for Revroute's web app.
- `tinybird`: Revroute's Tinybird configuration.
- `tsconfig`: The TypeScript configuration for Revroute's web app.
- `ui`: Revroute's UI component library.
- `utils`: A collection of utility functions and constants used across Revroute's codebase.

## How `app.revroute.ru` works

Revroute's web app is built with [Next.js](https://nextjs.org) and [TailwindCSS](https://tailwindcss.com).

It also utilizes code from the `packages` directory, specifically the `@dub/ui` and `@dub/utils` packages.

All of the code for the web app is located in here: [`main`/apps/web/app/app.revroute.ru](https://github.com/dubinc/dub/tree/main/apps/web/app/app.revroute.ru). This is using the Next.js [route group pattern](https://nextjs.org/docs/app/building-your-application/routing/route-groups).
...
## How link redirects work on Revroute

Link redirects on Revroute are powered by [Next.js Middleware](https://nextjs.org/docs/app/building-your-application/routing/middleware).
...
## Running Revroute locally

To run Revroute locally, you'll need to set up the following:

- A [Tinybird](https://www.tinybird.co/) account
- An [Upstash](https://upstash.com/) account
- A [PlanetScale](https://planetscale.com/)-compatible MySQL database

Watch this video from our friends at Tinybird to learn how to set up Revroute locally:
...
First, clone the [Revroute repo](https://d.to/github) into a public GitHub repository.
