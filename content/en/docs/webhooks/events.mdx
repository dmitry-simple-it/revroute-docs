...
This page lists every event you can subscribe to. The outer envelope (`id`, `event`, `createdAt`, `data`) is the same for all events — only the `data` object differs. See [Webhooks overview](/docs/webhooks) for transport details.
...
Fires when a [lead event](/docs/concepts/attribution#stage-2-lead-tracking) is recorded against a click.
...
Fires when a [sale event](/docs/quickstart/server#tracking-sale-events) is recorded.
