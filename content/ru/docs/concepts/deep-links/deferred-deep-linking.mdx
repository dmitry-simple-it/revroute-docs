...
<Note>
  Диплинки требуют подписки на [тарифный план Pro](https://revroute.ru/pricing) или
  выше.
</Note>
...
5. Приложение извлекает диплинк из URL реферера и отслеживает событие открытия диплинка через эндпоинт `/track/open`
...
```
https://play.google.com/store/apps/details?id=com.example.app&referrer=deepLink%3Dhttps%253A%252F%252Fgo.revroute.ru%252Fgps
```
...
class InstallReferrerTracker {
  ...
  extractDeepLinkFromReferrer(referrerUrl) {
    try {
      ...
      // e.g. for referrer=deepLink%3Dhttps%253A%252F%252Fgo.revroute.ru%252Fgps
      // the deep link is https://go.revroute.ru/gps
...
5. Приложение вызывает эндпоинт `/track/open` с параметром `deepLink` в теле запроса напрямую или через поддерживаемый мобильный SDK RevRoute.
...
4. После установки приложение вызывает эндпоинт `/track/open` с параметром `dubDomain` в теле запроса напрямую или через поддерживаемый мобильный SDK RevRoute.
...
- [Введение в REST API RevRoute](/docs/api-reference/introduction)
