...
description: "Learn how to track sales conversion events with Google Tag Manager and Revroute"
...
Revroute's [Google Tag Manager integration](https://revroute.ru/integrations/google-tag-manager) allows you to track conversion events directly from Google Tag Manager.

## Prerequisites

<EnableConversionTracking />

## Install Revroute via Google Tag Manager

There are 2 steps to install the Revroute Analytics script via Google Tag Manager:

1. [Add Revroute Analytics script to GTM](#step-1-add-dub-analytics-script-to-gtm)
2. [Create User-Defined Variable for dub_id Cookie](#step-2-create-user-defined-variable-for-dub_id-cookie)

### Step 1: Add Revroute Analytics script to GTM

First, you'll need to add the Revroute Analytics script to your website using Google Tag Manager.
...
<Warning>
  ...
  key](/docs/api-reference/authentication#publishable-keys) from your Revroute
  workspace (under the [Analytics settings
  page](https://app.revroute.ru/settings/analytics))
</Warning>

Configure the tag to fire on **All Pages** by setting the trigger to **All Pages - Page View**.

Name this tag "Revroute Analytics script" and save it.

### Step 2: Create User-Defined Variable for dub_id Cookie

To read the `dub_id` cookie that Revroute Analytics script sets, you'll need to create a User-Defined Variable in Google Tag Manager.
...
- **Variable Name**: Name it "Revroute ID Cookie"

Click **Save** to create the variable.

<Note>
  This variable will automatically read the `dub_id` cookie value set by the Revroute
  Analytics script. You can use this variable in your tags to pass the Revroute ID
...
Name this tag "Revroute Lead Tracking - Thank You Page" and save it.
...
Name this tag "Revroute Lead Tracking - Form Submission" and save it.
...
Name this tag "Revroute Sales Tracking - Order Confirmation" and save it.
...
Name this tag "Revroute Sales Tracking - Checkout Form" and save it.
...
2. **Using the Network tab** to see if requests are being sent to Revroute's analytics endpoint
3. **Viewing your Revroute dashboard** to confirm that sale events are appearing in your analytics
