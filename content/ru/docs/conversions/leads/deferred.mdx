...
description: "Узнайте, как отслеживать отложенные события конверсий лидов с помощью Dub"
---
...
Для этого нужно установить свойство `mode` в значение `deferred` при отслеживании события лида. При этом Dub по-прежнему будет отслеживать клиента и идентификатор клика, откуда он пришёл, но отложит создание фактического события лида на последующий запрос.
