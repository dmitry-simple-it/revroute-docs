---
title: "Self-hosting Revroute"
description: "An end-to-end guide on how to self-host Revroute – the open-source link attribution platform."
---

![Revroute Logo on a gradient background](/images/logo-background-gradient.png)

You can self-host Revroute on your own servers and cloud infrastructure for greater control over your data and design. This guide will walk you through the entire process of setting up Revroute on your own servers.
...
You'll also need a custom domain that you will be using for your Revroute instance, with an optional custom short domain for your links.
...
First, you'll need to clone the Revroute repo and install the dependencies.

<Steps>

  <Step title="Clone the repo">

First, clone the [Revroute repo](https://d.to/github) into a public GitHub repository. If you are planning to distribute the code or allow users to interact with the code remotely (e.g., as part of a hosted application), make sure to provide source access (including modifications) as required by the [AGPLv3 license](https://d.to/license).
...
In your newly-cloned Revroute repo, navigate to the `packages/tinybird` directory.
...
In your Revroute codebase, navigate to `apps/web/prisma/schema.prisma` and replace all the columns in the `DefaultDomains` model to the normalized version of your custom short domain (removing the `.` character).
...
Next, [create a new GitHub App](https://github.com/settings/applications/new). This will allow you to sign in to Revroute with your GitHub account.
...
Revroute stores user-generated assets in either S3 or S3-compatible services like [Cloudflare R2](https://cloudflare.com/r2). These include:

- Project logos
- User avatars
- [Custom Social Media Cards](/help/article/custom-link-previews) images

We recommend using [Cloudflare R2](https://cloudflare.com/r2) for self-hosting Revroute, as it's a more cost-effective solution compared to AWS S3. Here's how you can set it up:
...
Revroute uses Unsplash's API for the [Custom Social Media Cards](/help/article/custom-link-previews) feature. You'll need to set up an Unsplash application to get an access key.
...
This guide is meant to be a starting point for self-hosting Revroute. It currently depends on the following services to work:
...
In the future, we plan to make it easier to self-host Revroute by making these dependencies optional by swapping them out for native databases (e.g. mysql, redis, clickhouse, [GeoLite2](https://github.com/GitSquared/node-geolite2-redist) etc.)
