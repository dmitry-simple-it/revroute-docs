...
This page is the full error code reference: it maps every code to the specific symptom and fix — useful when you have a 4xx/5xx in front of you and want to act fast.
...
| Wrong token format | Server-to-server keys are `rr_*`; publishable keys `rr_pk_*`; OAuth `rr_access_token_*` |
| Token revoked or expired | Generate a new one in `Settings → Tokens` |
| Mixed up secret and publishable key | Server endpoints want `rr_*`; client endpoints want `rr_pk_*` |
...
| `503 service_unavailable` | Capacity / maintenance | Retry |
| `504 gateway_timeout` | Operation took too long | Retry once; if it persists, contact support |

If the error persists, [contact support](https://revroute.ru/contact/support) with the request method, URL and time, plus the response status and body. Log them so they are at hand:

```ts
const res = await fetch(url, opts);
if (!res.ok) {
  const body = await res.json().catch(() => null); // 502–504 responses may not be JSON
...
See also: [OAuth apps](/docs/oauth-apps) for token-based authentication, and [Troubleshooting](/docs/troubleshooting) for the rest of the guides.
