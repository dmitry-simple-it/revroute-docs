---
title: "Introduction to short links"
sidebarTitle: "Introduction"
description: "Learn how to use RevRoute to programmatically create, update, and delete links at scale."
---

# Introduction to short links

Links are the bread and butter of [RevRoute](https://revroute.ru).

Everything on RevRoute starts with a link. Whether you're creating:

- a handful of links for your marketing campaign
- hundreds of links for your [affiliate program](https://revroute.ru/)
- thousands of links, [created programmatically](/docs/concepts/links/bulk-operations), for your SMS campaign

In this guide, we'll cover the link model, how to create links, and more.

## The link model

The link model consists of the following properties:

| Property    | Description                                               | Example                          |
| ----------- | :-------------------------------------------------------- | :------------------------------- |
| `id`        | The unique identifier of the link (prefixed with `link_`) | `link_1KJ1H3PZGE081A22TC6PTQYJP` |
| `url`       | The destination URL of the link                           | `https://revroute.ru/`            |
| `shortLink` | The shortened version of the link (including https)       | `https://link.revroute.ru/claim` |
| `domain`    | The domain of the link                                    | `link.revroute.ru`               |
| `key`       | The short link slug                                       | `claim`                          |

For more advanced features — [conversion tracking](/docs/concepts/attribution), [tags and folders](/docs/concepts/links/organization), [bulk operations](/docs/concepts/links/bulk-operations) — see the guides linked from this page.

## Create a link

The `url` field, representing the destination URL, is the sole mandatory parameter required for the creation of a new short link.

<CodeGroup>

```javascript Node.js
import { RevRoute } from "dub";

export const dub = new RevRoute({
  token: process.env.REVROUTE_API_KEY,
});

const link = await dub.links.create({
  url: "https://google.com",
});
```

```go Go
package main

import(
  "context"
  dubgo "github.com/dubinc/dub-go"
  "github.com/dubinc/dub-go/models/operations"
  "log"
  "os"
)

func main() {
  ctx := context.Background()

  s := dubgo.New(
    dubgo.WithSecurity(os.Getenv("REVROUTE_API_KEY")),
  )

  res, err := s.Links.Create(ctx, &operations.CreateLinkRequestBody{
    URL: "https://google.com",
  })

  if err != nil {
    log.Fatal(err)
  }

  if res != nil {
    // handle response
  }
}
```

```python Python
import os
import dub
from dub.models import operations

d = dub.RevRoute(
  token=os.environ['REVROUTE_API_KEY'],
)

res = d.links.create(request={
  "url": "https://google.com",
})
```

```ruby Ruby
require 'dub'

s = ::OpenApiSDK::RevRoute.new
s.config_security(
  ::OpenApiSDK::Shared::Security.new(
    token: "REVROUTE_API_KEY",
  )
)

req = ::OpenApiSDK::Operations::CreateLinkRequestBody.new(
  url: "https://google.com",
)
```

```php PHP
declare(strict_types=1);

require 'vendor/autoload.php';

use RevRoute;
use RevRoute\Models\Operations;

$sdk = RevRoute\RevRoute::builder()->setSecurity('REVROUTE_API_KEY')->build();

$request = new Operations\CreateLinkRequestBody(
  url: 'https://google.com',
);

$response = $sdk->links->create(
  request: $request
);
```

```bash cURL {5}
curl --request POST \
  --url https://api.revroute.ru/links \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
  --data '{"url": "https://google.com"}'
```

</CodeGroup>


## Update a link

An existing link can be updated by providing the `id` to the `update` method. This method returns the updated link as a response.

You can use either the `linkId` or an `externalId` prefixed with `ext_` which is a unique identifier for the link in your own database to associate it with the link in RevRoute's system.

<CodeGroup>

```javascript Node.js
import { RevRoute } from "dub";

export const dub = new RevRoute({
  token: process.env.REVROUTE_API_KEY,
});

const link = await dub.links.update("link_1KJ1H3PZGE081A22TC6PTQYJP", {
  url: "https://www.google.uk", // new URL
});
```

```go Go
package main

import(
  "context"
  dubgo "github.com/dubinc/dub-go"
  "github.com/dubinc/dub-go/models/operations"
  "log"
  "os"
)

func main() {
  ctx := context.Background()

  s := dubgo.New(
    dubgo.WithSecurity(os.Getenv("REVROUTE_API_KEY")),
  )

  res, err := s.Links.Update(ctx, "link_1KJ1H3PZGE081A22TC6PTQYJP", &operations.UpdateLinkRequestBody{
    URL: "https://www.google.uk", // new URL
  })

  if err != nil {
    log.Fatal(err)
  }

  if res != nil {
    // handle response
  }
}
```

```python Python
import os
import dub
from dub.models import operations

d = dub.RevRoute(
  token=os.environ['REVROUTE_API_KEY'],
)

res = dub.links.update(link_id="link_1KJ1H3PZGE081A22TC6PTQYJP", request_body={
  "url": "https://www.google.uk", // new URL
})
```

```ruby Ruby
require 'dub'

s = ::OpenApiSDK::RevRoute.new
s.config_security(
  ::OpenApiSDK::Shared::Security.new(
    token: "REVROUTE_API_KEY",
  )
)

res = s.links.update(link_id="link_1KJ1H3PZGE081A22TC6PTQYJP", request_body={
  "url": "https://www.google.uk", // new URL
})
```

```php PHP
declare(strict_types=1);

require 'vendor/autoload.php';

use RevRoute;
use RevRoute\Models\Operations;

$sdk = RevRoute\RevRoute::builder()->setSecurity('REVROUTE_API_KEY')->build();

$requestBody = new Operations\UpdateLinkRequestBody(
  url: 'https://www.google.uk', // new URL
);

$response = $sdk->links->update(
  linkId: 'link_1KJ1H3PZGE081A22TC6PTQYJP',
  requestBody: $requestBody
);
```

```bash cURL
curl --request PATCH \
  --url https://api.revroute.ru/links/link_1KJ1H3PZGE081A22TC6PTQYJP \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
  --data '{"url": "https://www.google.uk"}'
```

</CodeGroup>


## Upsert a link

Upserting a link is a combination of creating and updating a link. If a link with the same URL already exists, return it (or update it if there are any changes). Otherwise, a new link will be created.

This allows you to use the upsert method without the necessity of checking for the link's existence beforehand.

<CodeGroup>

```javascript Node.js
import { RevRoute } from "dub";

export const dub = new RevRoute({
  token: process.env.REVROUTE_API_KEY,
});

const link = await dub.links.upsert({
  url: "https://google.com", // will always be the same short link
});
```

```go Go
package main

import(
  "context"
  dubgo "github.com/dubinc/dub-go"
  "github.com/dubinc/dub-go/models/operations"
  "log"
  "os"
)

func main() {
  ctx := context.Background()

  s := dubgo.New(
    dubgo.WithSecurity(os.Getenv("REVROUTE_API_KEY")),
  )

  res, err := s.Links.Upsert(ctx, &operations.UpsertLinkRequestBody{
    URL: "https://google.com",
  })

  if err != nil {
    log.Fatal(err)
  }

  if res != nil {
    // handle response
  }
}
```

```python Python
import os
import dub
from dub.models import operations

d = dub.RevRoute(
  token=os.environ['REVROUTE_API_KEY'],
)

res = d.links.upsert(request={
  "url": "https://google.com",
})
```

```ruby Ruby
require 'dub'

s = ::OpenApiSDK::RevRoute.new
s.config_security(
  ::OpenApiSDK::Shared::Security.new(
    token: "REVROUTE_API_KEY",
  )
)

req = ::OpenApiSDK::Operations::UpsertLinkRequestBody.new(
  url: "https://google.com",
)
```

```php PHP
declare(strict_types=1);

require 'vendor/autoload.php';

use RevRoute;
use RevRoute\Models\Operations;

$sdk = RevRoute\RevRoute::builder()->setSecurity('REVROUTE_API_KEY')->build();

$request = new Operations\UpsertLinkRequestBody(
  url: 'https://google.com',
);

$response = $sdk->links->upsert(
  request: $request
);
```

```bash cURL
curl --request POST \
  --url https://api.revroute.ru/links/upsert \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
  --data '{"url": "https://google.com"}'
```

</CodeGroup>

