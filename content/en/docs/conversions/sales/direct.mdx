...
description: Learn how to track sales without a prior lead event using Revroute
---
...
To track a direct sale, you need to pass the `clickId` parameter along with customer information when tracking the sale event. The `clickId` can be read from the `dub_id` cookie that's automatically set when a user clicks on your Revroute link.
