...
og:title: "How to set up deferred deep linking with Revroute"
...
![Deferred deep linking on Revroute](/images/deferred-deep-linking.png)
...
2. Revroute redirects the user to the App Store or Play Store by using [device targeting](/help/managing-your-links/links/device-targeting)
...
When Revroute redirects users to the Play Store, the referrer URL contains the deep link information in a nested structure. The referrer URL looks like this:
...
To get started, we recommend using the [quickstart guide](/docs/concepts/deep-links/quickstart) to set up your deep links on Revroute.
...
- [Revroute API Documentation](/docs/api-reference/introduction)

## iOS App Store

Unlike Android, iOS doesn't provide a built-in install referrer API. Revroute implements a hybrid approach combining **deterministic tracking** (clipboard-based) and **probabilistic tracking** (IP-based) to ensure reliable deferred deep linking on iOS.

### How iOS deferred deep linking works on Revroute

Revroute's iOS deferred deep linking solution provides two tracking approaches:
...
For the deterministic approach, when an iOS user who doesn't have your app installed clicks on your deep link, Revroute redirects them to a custom landing page:

![iOS deferred deep linking landing page](/images/ios-ddl-landing-page.jpg)
...
2. Revroute copies the deep link URL to the clipboard.
...
5. Your app calls the [`/track/open` endpoint](/docs/api-reference/endpoint/track-open) with the `deepLink` parameter in the request body either directly or via a supported [Revroute Mobile SDK](/docs/sdks/client-side-mobile/introduction).
6. Revroute returns the destination URL, and your app navigates the user to the appropriate screen (see [deep links quickstart](/docs/concepts/deep-links/quickstart) for more details).
...
3. Revroute has already tracked the click and stored the IP address at the time of deep link click.
4. After installation, your app calls the [`/track/open` endpoint](/docs/api-reference/endpoint/track-open) with the `dubDomain` parameter in the request body either directly or via a supported [Revroute Mobile SDK](/docs/sdks/client-side-mobile/introduction) .
5. Revroute matches the user using IP-based tracking and returns the destination URL.
...
- **IP-based tracking**: Used when no deep link is found in the clipboard or the user declined paste permissions. The app sends only the `dubDomain` parameter, allowing Revroute to match the user based on their IP address.
...
- [Revroute Deep Links Quickstart](/docs/concepts/deep-links/quickstart)
