...
<Steps>
  ...
  <Step title="Расширьте пакет `@dub/analytics` хуком `useUser` из Clerk">

    Расширьте [пакет `@dub/analytics`](/docs/sdks/client-side/introduction) серверным действием `trackLead`.
    ...
    Затем добавьте компонент `DubAnalytics` в корневой layout приложения:

    ```tsx app/layout.tsx
    import { DubAnalytics } from "@/components/analytics-overview";
  ...
  <Step title="Реализуйте серверное действие `trackLead`">
    ...
    export async function trackLead({
      ...
      try {
        ...
        const dubId = cookieStore.get("rr_id")?.value;
    ...
    export async function POST(req: NextRequest) {
      ...
      const dubId = req.cookies.get("rr_id")?.value;
      if (dubId) {
        // Send lead event to Revroute
        await dub.track.lead({
          clickId: dubId,
