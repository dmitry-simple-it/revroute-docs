With [RevRoute](https://revroute.ru/), you can build an embedded referral dashboard that lives directly inside your application in just a few lines of code.
...
<Frame>
  <video
    src="https://s3.revroute.ru/dub-public/assets/cms/dub-partners-whitelabeled-embed.mp4"
    autoPlay
    loop
    muted
    controls
...
In this guide, we'll walk you through the steps to get started with RevRoute's embedded referral dashboard feature.
...
Before we dive in, here's an open-source example app showing RevRoute's embedded referral dashboard in action: [demo.revroute.ru](https://demo.revroute.ru)
...
<Card
  ...
  color="#333333"
>
  View the source code for [demo.revroute.ru](https://demo.revroute.ru) on GitHub.
</Card>
...
```js JavaScript
const response = await fetch("https://api.revroute.ru/tokens/embed/referrals", {
  method: "POST",
  body: JSON.stringify({
    tenantId: user.id, // the user's ID within your application
    partner: {
...
const iframeUrl = "https://app.revroute.ru/embed/referrals";
