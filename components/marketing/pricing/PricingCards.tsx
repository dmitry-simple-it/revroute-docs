...
const CheckIcon = () => (
  <span className="inline-block shrink-0 w-[18px] h-[18px] mt-0.5 rounded-full bg-[var(--accent)]" style={{
    ...
    mask: `url("data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2020/svg' viewBox='0 0 24 24' fill='none' stroke='white' stroke-width='3' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpath d='M20 6L9 17l-5-5'/%3E%3C/svg%3E") center/12px no-repeat`,
  }} />
)
...
const partnerPlans: PlanCard[] = [
  {
    tier: 'Business',
    prices: { monthly: '2 950', annual: '2 450' },
    currency: '₽',
    period: '/ мес',
  ...
  {
    tier: 'Advanced',
    prices: { monthly: '9 999', annual: '8 299' },
    currency: '₽',
    period: '/ мес',
    ...
    features: [
      { bold: '1 500 000 ₽', rest: ' выплат партнёрам/мес' },
...
const linksPlans: PlanCard[] = [
  ...
  },
  {
    ...
    features: [
      ...
      'Встроенные парольная защита и маскирование',
  ...
  return (
    <div className={`grid grid-cols-1 ${colsClass} gap-5 mt-12 max-w-[480px] lg:max-w-none mx-auto`}>
      {plans.map((plan) => {
        ...
        const displayAmount = plan.prices ? plan.prices[billing] : undefined
        const priceNote = plan.amountCustom
          ? 'Условия под ваш бизнес'
          : isFree
          ? 'Старт без карты'
          : billing === 'annual'
          ? 'При оплате за год. Экономия 17%'
        ...
        return (
          ...
          >
            ...
            <p className="text-[12px] text-[var(--text-dim)] mb-5">{priceNote}</p>
