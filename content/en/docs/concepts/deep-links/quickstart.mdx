---
title: Quickstart
description: "Learn how to use Revroute to create deep links for your mobile app (with native support for React Native, iOS, and Android)."
---

<Note>
  Deep links require a [Pro plan](https://revroute.ru/pricing) subscription or
  higher.
</Note>

On Revroute, you can create deep links that lets you to redirect users to a specific page within your mobile application.
...
## Step 1: Configure your deep link domains on Revroute

Before you can create deep links, you need to configure your deep link domains in your Revroute workspace. This involves adding a custom domain that will be used for your deep links and configuring your deep link configuration files.

### Add a custom domain

First, you'll need to add a custom domain to your Revroute workspace. Navigate to your [workspace domain settings](https://app.revroute.ru/settings/domains) and click **Add Domain**.

You can use a domain you already own, or leverage our [free .link domain offer](/help/managing-your-links/custom-domains/free-dot-link-domain) to register a custom domain like `yourcompany.link` and use it as your deep link domain.

### Set up your deep link configuration files

Once you've set up your custom domain, you'll need to upload your deep link configuration files to Revroute, which we'll host under the `/.well-known/` directory of your domain.

To do that, go to your [workspace domain settings](https://app.revroute.ru/settings/domains) and click on the edit button for your custom domain:
...
**Option 1**: Handle the deep link using a supported Revroute Mobile SDK (iOS & React Native only)
...
To do this, make a `POST` request to the `/track/open` endpoint with the following body:
...
2. Track the open event via the `/track/open` endpoint
