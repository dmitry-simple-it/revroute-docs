---
title: "NextAuth.js"
description: "Learn how to track lead conversion events with NextAuth.js and RevRoute"
---
...
2. If the user is a new sign up, check if the `rr_id` cookie is present.
3. If the `rr_id` cookie is present, send a lead event to RevRoute using `revroute.track.lead`
4. Delete the `rr_id` cookie.

Under the hood, RevRoute records the user as a customer and associates them with the click event that they came from. The user's unique ID is now the source of truth for all future events – hence why we don't need the `rr_id` cookie anymore.
