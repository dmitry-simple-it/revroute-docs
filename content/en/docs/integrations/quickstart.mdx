...
description: "Learn how to authenticate users with OAuth 2.0 for your Revroute integration."
---

Integrations allow you to extend the capabilities of Revroute and seamlessly connect with third-party platforms and services.

Already-built integrations are listed in the [integrations catalog](/integrations); this page is about building your own.

You can build your own integrations with Revroute using our API.

1. Read the documentation on how to create links or [track sale conversions](/docs/quickstart/server#tracking-sale-events).
2. Learn how to install the [client-side tracking script](/docs/quickstart/client).
3. [Reach out to us](/contact/support) to feature your integration in the integrations marketplace.

In this guide, you will learn how to create and manage integrations on Revroute, allowing you to incorporate Revroute's link attribution platform into your application.

## Integrating via OAuth 2.0 (recommended)

Revroute supports OAuth 2.0 authentication, which is **recommended** if you build integrations extending Revroute's functionality.
...
<Info>
  ...
  not available in the Revroute SDKs. You'll need to call these endpoints directly
  using HTTP requests or an OAuth client library.
</Info>

### Set up OAuth 2.0

Here is a step-by-step guide on how to set up OAuth 2.0 authentication with Revroute.

<Steps>
  <Step title="Create an OAuth2 application in Revroute">
    - Go to the [OAuth Apps tab](https://app.revroute.ru/settings/oauth-apps) in your workspace.
  ...
  <Step title="Redirect users to authorization URL">
    When you want to authenticate a user, you need to redirect them to the Revroute OAuth authorization URL.
  ...
  <Step title="Exchange code for an access token">
    The `code` parameter is returned in the query string when the user is redirected back to your application. You can exchange this code for an access token by making a POST request to the Revroute OAuth token URL.

    ```
    POST https://api.revroute.ru/oauth/token
    ```
    ...
    <Tabs>
      <Tab title="With PKCE (recommended)">
        ...
        ```js
        await fetch("https://api.revroute.ru/oauth/token", {
          method: "POST",
          headers: {
        ...
        For example, the [Revroute Raycast extension](https://github.com/raycast/extensions/tree/main/extensions/dub) uses PKCE to authenticate users.
      </Tab>
    ...
    ```json
    {
      "access_token": "rr_access_token_ae8ebf6f97e6200d886ef48a5...",
  ...
  <Step title="Retrieve user and workspace info">
    ...
    ```
    GET https://api.revroute.ru/oauth/userinfo
    ```
    ...
    ```bash
    curl --request GET \
      --url https://api.revroute.ru/oauth/userinfo \
  ...
  <Step title="Make an API request with the access token">
    Once you have obtained a valid access token, you can use it to make requests to the Revroute API.

    You can initialize Revroute SDK with the access token.

    Here is an example of how you can create a link using the Revroute TypeScript SDK:
  ...
  <Step title="Refresh the access token">
    Revroute access tokens are short-lived, depending on the `expires_in` value (the default value is **7,200 seconds**, or **2 hours**). Revroute will respond with `401 Unauthorized` if you try to use an expired access token.

    To refresh the access token, you need to make a POST request to the Revroute OAuth token URL with the `refresh_token` you obtained when exchanging the code for an `access_token`.
...
You can request access to specific scopes when redirecting users to the Revroute OAuth authorization URL. Scopes are permissions that the user needs to grant to your application.

Revroute supports the following scopes for OAuth 2.0:
...
Revroute also supports API key authentication; however, it is **not recommended** for building integrations. It should only be used for internal integrations or personal projects that do not require user consent.
