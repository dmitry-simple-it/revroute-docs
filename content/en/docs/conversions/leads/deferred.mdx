...
description: Learn how to track a deferred lead conversion event with Revroute
---
...
To do this, you'll need to set the `mode` property to `deferred` when tracking the lead event. With this, Revroute will still track the customer and the click ID they came from, but defer the actual lead event creation to a subsequent request.
